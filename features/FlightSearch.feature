Feature: FlightSearch
  ...
  Scenario Outline: El usuario está planeando vuelo
    ...
    And El usuario selecciona el lugar de destino con el dato "<destination>" y hace click en el primer resultado
    And El usuario selecciona el calendario de ida y busca el dato "<going>"
    And El usuario selecciona una fecha de ida en el calendario que coincida con los datos de "<going>" y los selecciona haciendo click
    And El usuario selecciona una fecha de vuelta en el calendario que coincida con los datos de "<lap>" y los selecciona haciendo click
    ...
    Then El usuario tiene los datos de "<origin>", "<destination>", "<going>", "<lap>", "<adult>", "<children>" y "<baby>" y está listo para buscar un vuelo y pasar a seleccionar el horario

    Examples:
      | origin   | destination | going      | lap        | adult | children | baby | message                             |
